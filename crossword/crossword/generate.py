...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        """
        x_y_overlap = self.crossword.overlaps[x,y]
        revised = False
        ...
        if(x_y_overlap):
            ...
            for domainx in self.domains[x]:
                ...
                for domainy in self.domains[y]:
                    if(domainx[a] == domainy[b] and domainx != domainy):
                        possible = True
                        break
                if(not possible):
                    nodes_to_remove.add(domainx)
                    revised = True
            if(nodes_to_remove):
                self.domains[x] -= nodes_to_remove
    ...
    def assignment_complete(self, assignment):
        ...
        for value in self.crossword.variables:
            if(value not in assignment.keys()):
                return False
        return True
        ...
        for variable in assignment:
            ...
            for variable2 in assignment:
                if(variable != variable2):
                    ...
                    if(overlap):
                        a,b = overlap
                        if(assignment[variable][a] != assignment[variable2][b]):
                            return False
    ...
    def select_unassigned_variable(self, assignment):
        ...
        for variables in self.crossword.variables:
            if variables not in assignment:
                which.append([variables,len(self.domains[variables]),len(self.crossword.neighbors(variables))])
        ...
        if(which):
            for i in range(len(which)):
                ...
                if(variable[1] > best):
                    ...
                    best = variable[1]
                    dvojka = variable[2]
                elif(variable[1] == best):
                    if(variable[2] > dvojka):
                        unass = variable[0]
                        dvojka = variable[2]
    ...
    def backtrack(self, assignment):
        ...
        for value in self.order_domain_values(var,assignment):
            ...
            newass[var] = value
            if(self.consistent(newass)):
                result = self.backtrack(newass)
                if(result):
                    return result
